...
from procrustes_mapper import ProcrusteanMapper
...
def timesegments_classification(
    ...
    while True:
        ...
        if hyper is 'h':
            if len(str(kval))==1:
                hyper_ = copy.deepcopy(Hyperalignment(alignment = ProcrusteanMapper(k=0,scaling=True,reduction=False,oblique=False, Q= None)))
                mappers = hyper_(dss_train)
            else: 
                err_hyp_ts = []
                for k in kval:
                    hyp_ts = timesegments_classification_single(dss_train, hyper = 'gpa',
                                                          part1=HalfPartitioner(), 
                                                          part2=NFoldPartitioner(attr='subjects'), 
                                                          window_size=6, overlapping_windows=True, 
                                                          distance='correlation', do_zscore=False, k = k,
                                                          Q = Q)
                    err_hyp_ts.append(hyp_ts)
                    
                mean_hyp_err = [np.mean(ets) for ets in err_hyp_ts]
                k_cv_hyp = zip(kval,mean_hyp_err)
                khat_hyp = min(k_cv_hyp, key = lambda t: t[1])[0]
                hyper_ = copy.deepcopy(Hyperalignment(alignment = ProcrusteanMapper(k=khat_hyp,scaling=True,reduction=False,oblique=False, Q= Q)))
                mappers = hyper_(dss_train)
        else:
            if hyper is 'gpa':
                if len(str(kval))==1:
                   hyper_ = copy.deepcopy(ProMisesModel(maxIt =maxIt, t = t, k = kval, Q = None, ref_ds = ref_ds, scaling=scaling, 
                                                          reflection = reflection, subj=subj))
                   mappers = hyper_.gpa(dss_train)[1] 
                else:
                    err_ts = []
                    for k in kval:
                        gpa_ts = timesegments_classification_single(dss_train, hyper = 'gpa',
                                                          part1=HalfPartitioner(), 
                                                          part2=NFoldPartitioner(attr='subjects'), 
                                                          window_size=6, overlapping_windows=True, 
                                                          distance='correlation', do_zscore=False, k = k,
                                                          maxIt = maxIt, t = t, Q = Q, ref_ds = ref_ds,  scaling=scaling, 
                                                          reflection = reflection, subj=subj)
                        err_ts.append(gpa_ts)
                    
                    mean_err = [np.mean(ets) for ets in err_ts]
                    k_cv = zip(kval,mean_err)
                    khat = min(k_cv, key = lambda t: t[1])[0]
                    hyper_ = copy.deepcopy(ProMisesModel(maxIt =maxIt, t = t, k = khat, Q = Q, ref_ds =ref_ds, scaling=scaling, reflection = reflection, subj=subj))
                    mappers = hyper_.gpa(dss_train)[1] 
            else:
